...
/**
	...
	provided dby the instructional team. For this file we will have to write the helper functions
	only and everything else is provided.
...
class BSTree                                     // KeyType : key field
{
  ...
  protected:
    ...
    void showHelper ( BSTreeNode *p, int level ) const; //provided
    void insertHelper( BSTreeNode* &ptr, const DataType& newDataItem ); //to insert stuff
	void clearHelper( BSTreeNode* &ptr ); // to clear the tree
    void writeKeysHelper( BSTreeNode* ptr ) const; //write ethe info
    void copyHelper( BSTreeNode* &ptr, BSTreeNode* sourcePtr ); //to copy [used for copy constructor and assignment operator]
	
    bool removeHelper( BSTreeNode* &ptr, const KeyType& deleteKey );//to remove stuff
